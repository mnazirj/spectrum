<template>
  <div class="">
    ...
    <div id="body" class="d-flex w-100 justify-content-center">
      <DataTable :value="filteredItems" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" removableSort :class="['custom-datatable text-nowrap', isEng ? 'ltr' : 'rtl']" :style="{ width: '95%' }" :loading="items == null">
        ...
        <Column :header="$t('dash.sub-category.sub-category')">
          <template #body="slotProps">
            <div class="">
              <span v-if="isEng">{{ slotProps.data.sub_categories.title }}</span>
  ...
  <!-- Create Dialog -->
  <Dialog v-model:visible="createDialog" :modal="true" :closable="true" :header="$t('dash.item.create_a_item')" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
    <Form v-slot="$form" :resolver="createResolver" @submit="onFormSubmitCreate">
      <div class="w-100 d-flex justify-content-center align-items-center flex-wrap">
        <div class="section w-100 d-flex align-items-center flex-wrap m-3">
          <div>
            <FloatLabel variant="on">
              <InputText id="en-name" type="text" name="title" v-model="newItem.title" fluid />
              <label for="en-name"
          ...
          <div>
            <FloatLabel variant="on">
              <InputText id="ar-name" type="text" name="arName" v-model="newItem.arName" fluid />
              <label for="ar-name"
        ...
        <!-- Sub-Category -->
        <div class="w-100 d-flex justify-content-center flex-wrap align-items-center mt-2">
          <Select v-model="newItem.subCategoryId" optionValue="id" :options="subCategories" name="sub_category" optionLabel="title" :placeholder="$t('dash.item.select_a_sub-category')" class="w-50" />
          <Message v-if="$form.sub_category?.invalid" severity="error" size="small" variant="simple" class="w-100 d-flex justify-content-center">{{ $t($form.sub_category.error?.message) }}</Message>
        </div>
      </div>
  ...
  <!-- Edit Dialog -->
  <Dialog v-model:visible="editDialog" :modal="true" :closable="true" :header="`${$t('dash.edit')} ${$t('dash.item.item')}  ' ${isEng ? currentData.title : currentData.arName}'`" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
    <Form v-slot="$form" :resolver="editResolver" @submit="onFormSubmitEdit">
      <div class="w-100 d-flex justify-content-center align-items-center flex-wrap">
        <div class="section w-100 d-flex align-items-center flex-wrap m-3">
          <div>
            <FloatLabel variant="on">
              <InputText id="en-name" name="title" type="text" v-model="currentData.title" fluid />
              <label for="en-name"
                ><i class="fa-solid fa-heading mx-1" /><span class="mx-1">{{ $t("dash.name_in_english") }}</span></label
              >
            </FloatLabel>
          ...
          <div>
            <FloatLabel variant="on">
              <InputText id="ar-name" name="arName" type="text" v-model="currentData.arName" fluid />
              <label for="ar-name"
                ><i class="fa-solid fa-heading mx-1" /><span class="mx-1">{{ $t("dash.name_in_arabic") }}</span></label
              >
            </FloatLabel>
        ...
        <div class="w-100 d-flex justify-content-center align-items-center mt-2">
          <Select v-model="currentData.subCategoryId" optionValue="id" :options="subCategories" optionLabel="title" :placeholder="$t('dash.item.select_a_sub-category')" name="sub_category" class="w-50" />
          <Message v-if="$form.sub_category?.invalid" severity="error" size="small" variant="simple" class="w-100 d-flex justify-content-center">{{ $t($form.sub_category.error?.message) }}</Message>
        </div>
      </div>
      <div class="d-flex justify-content-end">
        <div class="d-flex m-1 mt-3">
          <Button :label="$t('dash.update')" icon="pi pi-file-arrow-up" severity="info" type="submit" :loading="loadingData"></Button>
        </div>
      </div>
    </Form>
  </Dialog>
  <!-- Delete Dialog -->
  <Dialog v-model:visible="deleteDialog" :modal="true" :closable="true" :header="`${$t('dash.delete')} ${$t('dash.item.item')}  ' ${isEng ? currentData.title : currentData.arName}'`" :style="{ width: '35rem' }" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
    <span v-if="isEng"> {{ $t("dash.delete_question1") }} {{ $t("dash.item.item") }} "{{ currentData.title }}" {{ $t("dash.?") }}</span>
    <span v-else> {{ $t("dash.delete_question1") }} {{ $t("dash.item.item") }} "{{ currentData.arName }}" {{ $t("dash.?") }}</span>
    <template #footer>
      <Button icon="pi pi-trash" :label="$t('dash.delete')" severity="danger" @click="deleteItem" :loading="loadingData"></Button>
    </template>
  </Dialog>
</template>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => item.title.toLowerCase().includes(searchValue.value.toLowerCase()) || item.arName.toLowerCase().includes(searchValue.value.toLowerCase()) || item.id.toString().includes(searchValue.value));
});
...
function updateItem() {
  loadingData.value = true;
  axios
    .put(`/control/modify/subitems/${currentData.value.id}`, currentData.value, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("_token")}`,
      },
    })
...
const createResolver = ({ values }) => {
  ...
  if (!values.sub_category) {
    errors.sub_category = [{ message: "dash.item.you_must_select_one_of_sub-categories" }];
  }
  return { errors };
};
const editResolver = () => {
