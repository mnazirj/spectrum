<template>
  <div class="">
    ...
    <div id="body" class="d-flex w-100 justify-content-center">
      <DataTable :value="filteredSubCategories" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" removableSort :class="['custom-datatable text-nowrap', isEng ? 'ltr' : 'rtl']" :style="{ width: '95%' }" :loading="subCategories == null">
  ...
  <!-- Create Dialog -->
  <Dialog v-model:visible="createDialog" :modal="true" :closable="true" :header="$t('dash.sub-category.create_a_sub-category')" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
    <Form v-slot="$form" :resolver="createResolver" @submit="onFormSubmitCreate">
      <div class="w-100 d-flex justify-content-center align-items-center flex-wrap">
        <div class="section w-100 d-flex align-items-center flex-wrap m-3">
          <div>
            <FloatLabel variant="on">
              <InputText id="en-name" type="text" name="title" v-model="newSubCategory.title" fluid />
              <label for="en-name"
          ...
          <div>
            <FloatLabel variant="on">
              <InputText id="ar-name" type="text" name="arName" v-model="newSubCategory.arName" fluid />
              <label for="ar-name"
        ...
        <div class="w-100 d-flex justify-content-center flex-wrap align-items-center mt-2">
          <Select v-model="newSubCategory.categoryId" name="category" optionValue="id" :options="categories" :optionLabel="isEng ? 'title' : 'arName'" :placeholder="$t('dash.sub-category.select_a_category')" class="w-50" />
          <Message v-if="$form.category?.invalid" severity="error" size="small" variant="simple" class="w-100 d-flex justify-content-center">{{ $t($form.category.error?.message) }}</Message>
        </div>
      </div>
      <div class="d-flex justify-content-end">
        <div class="d-flex m-1 mt-3">
          <Button :label="$t('dash.create')" icon="pi pi-plus" :loading="loadingData" type="submit"></Button>
        </div>
      </div>
    </Form>
  </Dialog>
  <!-- Edit Dialog -->
  <Dialog v-model:visible="editDialog" :modal="true" :closable="true" :header="`${$t('dash.edit')} ${$t('dash.sub-category.sub-category')} ' ${isEng ? currentData.title : currentData.arName}'`" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
      ...
      <div class="w-100 d-flex justify-content-center align-items-center flex-wrap">
        <div class="section w-100 d-flex align-items-center flex-wrap m-3">
          ...
          <div>
            ...
            <Message v-if="$form.arName?.invalid" severity="error" size="small" variant="simple" name="category" class="w-100">{{ $t($form.arName.error?.message) }}</Message>
          </div>
        </div>
        <!-- category -->
        <div class="w-100 d-flex justify-content-center align-items-center mt-2">
          <Select v-model="currentData.categoryId" optionValue="id" :options="categories" :optionLabel="isEng ? 'title' : 'arName'" :placeholder="$t('dash.sub-category.select_a_category')" class="w-50" />
          <Message v-if="$form.category?.invalid" severity="error" size="small" variant="simple" class="w-100 d-flex justify-content-center">{{ $t($form.category.error?.message) }}</Message>
        </div>
      </div>
      <div class="d-flex justify-content-end">
  ...
  <!-- Delete Dialog -->
  <Dialog v-model:visible="deleteDialog" :modal="true" :closable="true" :header="`${$t('dash.delete')} ${$t('dash.sub-category.sub-category')} '${isEng ? currentData.title : currentData.arName}'`" :style="{ width: '35rem' }" :breakpoints="{ '1199px': '85vw', '575px': '95vw' }" :dir="isEng ? 'ltr' : 'rtl'" dismissableMask>
    <span v-if="isEng"> {{ $t("dash.delete_question1") }} "{{ currentData.title }}" {{ $t("dash.from") }} {{ $t("dash.sub-category.sub-category") }} {{ $t("dash.?") }}</span>
    <span v-else> {{ $t("dash.delete_question1") }} "{{ currentData.arName }}" {{ $t("dash.from") }} {{ $t("dash.sub-category.sub-category") }} {{ $t("dash.?") }}</span>
    <template #footer>
      <Button icon="pi pi-trash" :label="$t('dash.delete')" severity="danger" @click="deleteSubCategory" :loading="loadingData"></Button>
    </template>
  </Dialog>
</template>
...
function updateSubCategory() {
  loadingData.value = true;
  axios
    .put(`/control/modify/subcategory/${currentData.value.id}`, currentData.value, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("_token")}`,
      },
    })
...
const createResolver = ({ values }) => {
  ...
  if (!values.category) {
    errors.category = [{ message: "dash.sub-category.you_must_select_one_of_cateogries" }];
  }
  return { errors };
};
const editResolver = () => {
