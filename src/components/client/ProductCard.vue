    ...
    <div class="container">
      ...
      <DataView :value="products" paginator :rows="12" v-if="!hasSlider">
        <template #empty>
          <div class="text-center col-lg-6 col-md-6 col-12 mx-auto">
            <Message severity="contrast"
              >There is no product matching the selected filter</Message
            >
          </div>
        </template>
        <template #list="slotProps">
          <div class="row">
            <div
              class="col-lg-4 col-md-4 col-12 h-100"
                ...
                <div class="card-body">
                  ...
                  <div>
                    <small class="text-muted"
                      >{{ item.sizes.length }} Sizes</small
                    >
                  </div>
                </div>
                <div class="card-footer border-0 bg-transparent">
                  <h4>LYD {{ formatPrice(item.price) }}</h4>
                </div>
              </div>
            </div>
          </div>
